...
def filter_tags(elements: list, taginfo: pd.DataFrame) -> list:
    ...
    for element in elements:
        ...
        if "layer" in tags.keys() and tags["layer"] < "0":
            # Feature not visible
            continue
        if "location" in tags.keys() and (
            tags["location"] == "underground" or tags["location"] == "underwater"
        ):
            # Feature not visible
            continue
        for key in tags.keys():
            if "tiger:" in key:
                ...
                if (
                    tgroup != "references"
                    and tgroup != "import"
                    and tgroup != "annotations"
                    and tgroup != "addresses"
                    and tgroup != "boundaries"
                ):
        ...
        if len(filtered_tags) != 0:
            if (
                "name" in filtered_tags.keys()
                and "noname" in filtered_tags.keys()
                and "yes" in filtered_tags["noname"]
            ):
...
def find_intersection(image_bbox: list, element: dict) -> dict:
    ...
    if check_closed_way(element):
            ...
            if intersection_shapely.geom_type == "MultiPolygon":
                ...
                for polygon in polygons:
                    ...
                    for point in list(polygon.exterior.coords):
                        ...
                        temp_intersection.append(
                            {
                                "lat": temp_intersection[0]["lat"],
                                "lon": temp_intersection[0]["lon"],
                            }
                        )
                ...
                intersection = [
                    ...
                    for point in list(intersection_shapely.exterior.coords)
                ]
                # If it was closed, it should be closed also now
                intersection.append(
                    {"lat": intersection[0]["lat"], "lon": intersection[0]["lon"]}
                )
    else:
        ...
        for i in range(0, len(geometry) - 1):  # considering segment by segment
            ...
            if not (intersection_line.is_empty):
                ...
                if (first != first_point) and (last == last_point):
                    ...
                    intersection.append(
                        {
                            "lat": intersection_line.coords[0][0],
                            "lon": intersection_line.coords[0][1],
                        }
                    )
                    ...
                    intersection.append(
                        {
                            "lat": intersection_line.coords[-1][0],
                            "lon": intersection_line.coords[-1][1],
                        }
                    )
                elif (first != first_point) and (last != last_point):
        ...
        if check_point_inside_bbox(geometry[-1], i_bbox):
            intersection.append(
                {"lat": geometry[-1]["lat"], "lon": geometry[-1]["lon"]}
            )
    ...
    for point in intersection:
        if type(point) == list:
            ...
            for p in point:
                if (
                    p["lat"] < image_bbox[0]
                    or p["lat"] > image_bbox[2]
                    or p["lon"] < image_bbox[1]
                    or p["lon"] > image_bbox[3]
                ):
                    raise ValueError("Intersection out of enclosing bbox!")
        else:
            # Single polygon intersection, or line intesection
            if (
                point["lat"] < image_bbox[0]
                or point["lat"] > image_bbox[2]
                or point["lon"] < image_bbox[1]
                or point["lon"] > image_bbox[3]
            ):
                raise ValueError("Intersection out of enclosing bbox!")
            ...
            if element["type"] == "node":
                ...
                if check_closed_way(element):
                    ...
                    for j in range(len(lat)):
                        ...
                        for i, center in enumerate(tiles_centers):
                            dist = math.sqrt(
                                (lat[j] - center[0]) ** 2 + (lon[j] - center[1]) ** 2
                            )
                            if dist < min_dist:
                                min_dist = dist
                                index = i
                        if index not in already_placed:
                            ...
                            element["tags"]["position"] = element["tags"]["position"][
                                :-5
                            ]
                    ...
                    for i, center in enumerate(tiles_centers):
                        dist = math.sqrt(
                            (nodes_inside[0]["lat"] - center[0]) ** 2
                            + (nodes_inside[0]["lon"] - center[1]) ** 2
                        )
                        if dist < min_dist:
                            min_dist = dist
                            indexstart = i
                    ...
                    for i, center in enumerate(tiles_centers):
                        dist = math.sqrt(
                            (nodes_inside[-1]["lat"] - center[0]) ** 2
                            + (nodes_inside[-1]["lon"] - center[1]) ** 2
                        )
                        if dist < min_dist:
                            min_dist = dist
                            indexend = i
                    ...
                    if indexstart == indexend:
                        ...
                        element["tags"]["position"] = (
                            "from "
                            + placing_identifiers[indexstart]
                            + " to "
                            + placing_identifiers[indexend]
                        )
...
def intersect_elements(
    filtered_elements: list[dict], image_bbox: list[float]
) -> list[dict]:
    """
...
def eliminate_duplicate_elements(elements: list[dict]) -> list[dict]:
    ...
    for i in range(len(filtered_elements)):
        ...
        for j in range(len(filtered_elements)):
            if i != j:
                if is_represented(
                    filtered_elements[i]["tags"], filtered_elements[j]["tags"]
                ):
                    to_add = False
        if to_add:
...
def remove_names(
    elements: list[dict], taginfo: pd.DataFrame, all_category: bool = True
) -> list[dict]:
    """
    ...
    for element in elements:
        if "layer" in element["tags"].keys() and element["tags"]["layer"] < "0":
            continue
        if "location" in element["tags"].keys() and (
            element["tags"]["location"] == "underground"
            or element["tags"]["location"] == "underwater"
        ):
            continue
        if all_category:
            new_tags = {}
...
def remove_names_element(
    element: dict, taginfo: pd.DataFrame, all_category: bool = True
) -> list[dict]:
    """
    This function removes the "name" and "alt_name" entries from the elements tags.
    It is used to avoid the model to learn the names of the elements, as they are too specific information for the model.
...
def quantize_boundary(
    boundary: list[list[float]], image_resolution: int, image_bbox: list[float]
) -> list[list[int]]:
    """
    ...
    for i, point in enumerate(boundary):
        ...
        pixel_coords = lat_lon_to_pixel_coords(
            image_bbox, lat, lon, (image_resolution, image_resolution)
        )
        if pixel_coords is not None:
            ...
            if i == 0:
                quantized_boundary.append(
                    (round(pixel_coords_x, 2), round(pixel_coords_y, 2))
                )
            else:
                if quantized_boundary[-1] != (
                    round(pixel_coords_x, 2),
                    round(pixel_coords_y, 2),
                ):
                    quantized_boundary.append(
                        (round(pixel_coords_x, 2), round(pixel_coords_y, 2))
                    )
...
def connect_elements(elements: list[dict]) -> list[dict]:
    ...
    elements_to_connect = [
        element
        for element in elements
        if element["type"] == "way" and not check_closed_way(element)
    ]
    indexes = [
        i
...
def filter_outside_elements(
    elements: list[dict], image_bbox: list[float]
) -> list[dict]:
    """
    ...
    bbox_reduced = [
        image_bbox[0] + 0.03 * lat_width,
        image_bbox[1] + 0.03 * lon_width,
        image_bbox[2] - 0.03 * lat_width,
        image_bbox[3] - 0.03 * lon_width,
    ]
    bbox_reduced_poly = Polygon(
        [
            (bbox_reduced[0], bbox_reduced[1]),
            (bbox_reduced[0], bbox_reduced[3]),
            (bbox_reduced[2], bbox_reduced[3]),
            (bbox_reduced[2], bbox_reduced[1]),
            (bbox_reduced[0], bbox_reduced[1]),
        ]
    )
    new_elements = []
...
def post_process_osm_data(base_folder_dataset: str):
    ...
    postprocessed_data_path = os.path.join(
        base_folder_dataset, "postprocessed_osm_data"
    )
    ...
    for file in os.listdir(raw_data_path):
        if file.endswith(".json"):
            if os.path.exists(os.path.join(postprocessed_data_path, file)):
                pbar.update(1)
                continue
...
if __name__ == "__main__":
    post_process_osm_data("test_scraping/")
